...
from .common import DatasetFromList, MapDataset, ClassGroupedDataset
...
__all__ = [
    ...
    "get_detection_cam_aux_dataset_dicts",
...
def build_detection_train_loader(cfg, mapper=None):
    ...
    dataset_dicts = get_detection_dataset_dicts(
        cfg.DATASETS.TRAIN,  # ("cervix_train",)
        filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,  # true
        min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE   # 1
    ...
    group_bin_edges = [1] if cfg.DATALOADER.ASPECT_RATIO_GROUPING else []  # True
...
def get_detection_cam_aux_dataset_dicts(
        dataset_names, filter_empty=True, min_keypoints=0, proposal_files=None
):
    """
    Load and prepare dataset dicts for instance detection/segmentation and semantic segmentation.
    ...
    for i in range(len(dataset_dicts)):
        if dataset_dicts[i]['image_label'] == 1:
            norm_dataset_dicts.append(dataset_dicts[i])
        elif dataset_dicts[i]['image_label'] == 0:
            hsil_dataset_dicts.append(dataset_dicts[i])
        else:
            raise ValueError("Unknown image label: {}".format(dataset_dicts[i]['image_label']))
    ...
    if proposal_files is not None:
        ...
        hsil_dataset_dicts = [
            load_proposals_into_dataset(dataset_i_dicts, proposal_file)
            for dataset_i_dicts, proposal_file in zip(hsil_dataset_dicts, proposal_files)
        ]

    has_instances = "annotations" in hsil_dataset_dicts[0]
    # Keep images without instance-level GT if the dataset has semantic labels.
    if filter_empty and has_instances and "sem_seg_file_name" not in hsil_dataset_dicts[0]:
        hsil_dataset_dicts = filter_images_with_only_crowd_annotations(hsil_dataset_dicts)

    if min_keypoints > 0 and has_instances:
        hsil_dataset_dicts = filter_images_with_few_keypoints(hsil_dataset_dicts, min_keypoints)

    if has_instances:
        try:
            class_names = MetadataCatalog.get(dataset_names[0]).thing_classes
            check_metadata_consistency("thing_classes", dataset_names)
            print_instances_class_histogram(hsil_dataset_dicts, class_names)
        except AttributeError:  # class names are not available for this dataset
            pass

    dataset_dicts = norm_dataset_dicts + hsil_dataset_dicts
    return dataset_dicts


def build_detection_cam_aux_train_loader(cfg, mapper=None):
    """
       A data loader is created by the following steps:
    ...
    dataset_dicts = get_detection_cam_aux_dataset_dicts(
        cfg.DATASETS.TRAIN,  # ("cervix_train",)
        filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,  # true
        min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE  # 1
        if cfg.MODEL.KEYPOINT_ON  # False
    ...
    data_loader = ClassGroupedDataset(data_loader, images_per_worker)
    return data_loader
